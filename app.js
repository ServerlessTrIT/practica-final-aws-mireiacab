...
const API_KEY="c5jjqaobtG1x0ivp6anFK3e7O8VShrN76qSoi28k"
const API_URL = "https://ci16cl19qe.execute-api.eu-central-1.amazonaws.com/dev/";
const API_LOGIN = API_URL+"login";
const API_SIGNUP = API_URL+"signup";
const API_CONFIRMSIGNUP = API_URL+"confirmsignup";
const API_BOOKS = API_URL+"games";
...
function  newBookPage(){
  ...
  content+='<form id="formBook"><input type="text" name="id" placeholder="ID" id="id"><input type="text" name="title" id="title" placeholder="Título"><button type="submit" value="Enviar" id="btnNewBook">Enviar</button></form>';
  return content;
}

function booksPage(){
  content='<h1>Videojuegos</h1><br/><button id="linkNewBook">Nuevo juego</button><br/>';
  content+='<table border="1"><tr><th>ID</th><th>Título</th><th>Eliminar</th></tr>';
  for (i = 0; i< books.length; i++){
    content+='<tr><td>'+books[i].id+'</td><td>'+books[i].title+'</td><td><button id="id_'+books[i].id+'">X</button></td></tr>';
  }
  content+='</table>'
  return content;
}
...
/* CARGAR VISTAS */
  ...
  if (window.location.pathname === '/games') {
    ...
    content = '<h1>¡Hola Sonia!</h1><h2>Esta es la web de Mireia para el curso de Arquitectura serverless con AWS</h2>';
...
function init(){
  ...
  $("body").on("click","button[id^='id']",deleteBook);
